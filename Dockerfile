...
RUN echo "Main-Class: license_injector.LicenseInjector" > manifest.mf && \
    jar cfm /app/LicenseInjector.jar manifest.mf -C /app/classes license_injector || true && \
    jar cf /app/patcher_java.jar -C /app/classes com/aetheria/patchers || true

FROM python:3.11-slim

WORKDIR /app
RUN apt-get update && apt-get install -y openjdk-21-jre-headless && rm -rf /var/lib/apt/lists/*
COPY --from=builder /app/LicenseInjector.jar /app/license_injector/ 2>/dev/null || true
COPY --from=builder /app/patcher_java.jar /app/patcher_java/ 2>/dev/null || true
